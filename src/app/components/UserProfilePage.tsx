...
export function UserProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<'events' | 'vault' | 'attendance'>('events');
        ...
        <div className="max-w-4xl mx-auto px-4 py-3 md:py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <span className="text-white font-bold hidden sm:block">ملفي الشخصي</span>
      ...
      <main className="flex-1 max-w-4xl mx-auto px-4 py-8 w-full">
        ...
        <div className="bg-card rounded-2xl border border-border shadow-sm mb-6 p-1.5 flex">
          {[
            ...
            { id: 'vault', label: 'خزنة الشهادات', icon: Award },
          ...
          <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div>
              <h3 className="text-lg font-bold flex items-center gap-2 mb-4">
                ...
                الفعاليات المسجلة ({registeredEvents.length})
              </h3>
              {registeredEvents.length === 0 ? (
                <EmptyState icon={Calendar} message="لا توجد فعاليات مسجلة حاليًا" />
              ) : (
            ...
            {waitlistEvents.length > 0 && (
              <div>
                <h3 className="text-lg font-bold flex items-center gap-2 mb-4">
                  ...
                  قائمة الانتظار ({waitlistEvents.length})
                </h3>
                <div className="space-y-3">
            ...
            {/* Attended with pending checkout */}
            {attendedEvents.length > 0 && (
              <div>
                <h3 className="text-lg font-bold flex items-center gap-2 mb-4">
                  <span className="w-2 h-6 bg-green-500 rounded-full"></span>
                  الفعاليات المحضورة ({attendedEvents.length})
                </h3>
                <div className="space-y-3">
                  {attendedEvents.map(event => {
                    const reg = myRegistrations.find(r => r.eventId === event.id)!;
                    const needsCheckout = reg.checkedIn && !reg.checkedOut;
                    return (
                      <div
                        key={event.id}
                        className={`bg-card border rounded-2xl p-4 flex gap-4 items-center shadow-sm cursor-pointer group transition-all ${
                          needsCheckout
                      ...
                      >
                        ...
                        <div className="flex flex-col items-end gap-1.5 shrink-0">
                          ...
                          {needsCheckout && (
                            <span className="flex items-center gap-1 px-3 py-1 bg-primary/10 text-primary rounded-lg text-xs font-bold border border-primary/20 animate-pulse">
                              تسجيل المغادرة
                            </span>
                          )}
                        </div>
        ...
        {activeTab === 'attendance' && (
          <div className="animate-in fade-in duration-300">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-4">
              <span className="w-2 h-6 bg-green-500 rounded-full"></span>
              سجل الحضور والمشاركة ({attendedEvents.length})
            </h3>
            {attendedEvents.length === 0 ? (
              <EmptyState
                icon={CheckCircle}
                message="لا يوجد سجل حضور حتى الآن"
                subMessage="سيظهر هنا تاريخ حضورك للفعاليات المكتملة"
              />
            ) : (
